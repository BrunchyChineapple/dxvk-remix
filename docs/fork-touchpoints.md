...
- **Inline tweak** at `RtxOptions` class body (moon-lighting strength sliders) — +10 LOC (Phase 1) + +18 LOC (Phase 3).
  *Phase 1 (2026-05-07) added `moonNeeStrength` (world-side master, default 1.0) and `moonAtmosphericCouplingStrength` (sky-side, default 1.0) RTX_OPTIONs. Phase 3 (2026-05-08) added per-path stylistic multipliers on top of the unified physical scaffold: `surfaceMoonBrightness` (default 8.0 — surface NEE only), `cloudMoonBrightness` (default 24.0 — cloud directional + ambient airglow), `haloMoonBrightness` (default 5.0 — disk halo Gaussian glow). Phase 3 defaults are empirically calibrated to approximate pre-Phase-2 visibility under FNV's tonemapper at `m.brightness=1.0`; setting all three to 1.0 yields architecturally-pure physical-neutral output. Consumed across `evalAtmosphereRadiance`, `evalClouds`, `evalMoonDisk`'s halo, and `sampleAtmosphereMoonLight`.*
